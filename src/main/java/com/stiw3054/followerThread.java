...
//This class we have 5 method(1.include getContentByJsoup(),which require html content from github
//                            2.getDivContentByJsoup(),which require the content what we need by Elementclass
//                            3.getAllName(),which get all follower's name,because the total of teacher follower are 166 and they take up 4 pages.Hence we need to separate four url to get total of four pages follower name.
//                            4.getLinksByJsoup(),which i set one more getlink,but this method only to collect how many follower links have in one page.
//                            5.run(),which is my thread method,and this method will call crawler function,and to request each follower information(such as how many follower,following,repositories and star)
public class followerThread extends Thread {
    private int i;
    public followerThread(int i){
        this.i = i;
    }
...
//    for(int i = 0;i<getEachLink().size();i++){
